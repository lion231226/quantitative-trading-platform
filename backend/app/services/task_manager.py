...
class TaskManager:
    ...
    async def _run_task(
        ...
        **kwargs: Any
        ...
        try:
            ...
            logger.info("异步任务开始执行", task_id=task_id)
            ...
            logger.info("异步任务执行完成", task_id=task_id)
            return result
            ...
            logger.error("异步任务执行失败", task_id=task_id, error=str(e))
            raise
    ...
    def cleanup_completed_tasks(self, older_than_hours: int = 24) -> int:
        ...
        logger.info("清理已完成的任务", removed_count=len(tasks_to_remove))
