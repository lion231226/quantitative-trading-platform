...
const PriceChart = forwardRef<any, PriceChartProps>(({
  ...
  const [config, setConfig] = useState<ChartConfig>({ ...DEFAULT_CONFIG, ...userConfig });
  ...
  const createMovingAverageDataset = useCallback((data: MovingAverageLine | MovingAverageLine[], color: string) => {
    // 处理单个对象或数组的情况
  ...
  const chartData = useMemo<ChartJSData<'line'>>(() => {
    ...
    const datasets = [
      ...
      ...optimizedData.movingAverages.map((ma, index) =>
        createMovingAverageDataset(ma, ['#10B981', '#F59E0B', '#06B6D4'][index % 3])
      ),
    ];
        ...
        if (elements.length > 0) {
          ...
          if (onPointClick) {
            ...
            onPointClick({
              ...
              data: chartData.datasets[clickedElement.datasetIndex]?.data[clickedElement.index],
        ...
        switch (format) {
          ...
          case 'csv':
            ...
            data.prices.forEach(price => {
              ...
              csvRows.push(`${price.timestamp},${price.open},${price.high},${price.low},${price.close},${signal ? signal.type : ''},${signal ? signal.price : ''}`);
            });
  ...
  const renderControls = () => (
    <div className="flex flex-wrap gap-4 p-4 bg-gray-50 border-b">
      <div className="flex items-center space-x-2">
        <label htmlFor="show-signals" className="text-sm font-medium">显示信号:</label>
        <input
          id="show-signals"
          type="checkbox"
          checked={config.showSignals}
          onChange={(e) => handleConfigChange({ showSignals: e.target.checked })}
          className="rounded"
        />
      </div>

      <div className="flex items-center space-x-2">
        <label htmlFor="show-ma" className="text-sm font-medium">显示均线:</label>
        <input
          id="show-ma"
          type="checkbox"
      ...
      {config.showMovingAverages && (
        <>
          <div className="flex items-center space-x-2">
            <label htmlFor="ma-type" className="text-sm font-medium">均线类型:</label>
            <select
              id="ma-type"
              value={config.movingAverageType}
              onChange={(e) => handleConfigChange({ movingAverageType: e.target.value as 'SMA' | 'EMA' })}
          ...
          <div className="flex items-center space-x-2">
            <label htmlFor="ma-period" className="text-sm font-medium">周期:</label>
            <input
              id="ma-period"
              type="number"
              value={config.movingAveragePeriod}
              onChange={(e) => handleConfigChange({ movingAveragePeriod: parseInt(e.target.value) || 20 })}
              min="5"
              max="200"
          ...
          <CardContent>
            <div className="space-y-2">
              ...
              <div className="flex justify-between">
                ...
                <span>{new Date(selectedSignal.timestamp).toLocaleString()}</span>
              </div>
              <div className="flex justify-between">
...
}) as React.ForwardRefExoticComponent<PriceChartProps & React.RefAttributes<any>>;
