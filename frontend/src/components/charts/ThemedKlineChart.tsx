...
const ThemedKlineChart: React.FC<ThemedKlineChartProps> = React.memo(
  ({
    ...
    const [baselineCurve, setBaselineCurve] = useState<FundCurveData | null>(
      null,
    );
    ...
    const chartConfig = useMemo(() => {
      ...
      return createKlineConfigFromTheme(currentTheme.colors, {
        height: rest.height,
        width: rest.width,
      });
    ...
    const handleBaselineUpdate = useCallback(
      (baseline: FundCurveData, metrics: PerformanceMetrics) => {
  ...
  (prevProps, nextProps) => {
    ...
    const basicPropsEqual = keysToCompare.every(
      (key) => prevProps[key] === nextProps[key],
    );
    const dataEqual =
      JSON.stringify(prevProps.data) === JSON.stringify(nextProps.data);
    const fundCurvesEqual =
      JSON.stringify(prevProps.fundCurves) ===
      JSON.stringify(nextProps.fundCurves);
    const priceDataEqual =
