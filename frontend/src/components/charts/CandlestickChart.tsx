...
export const CandlestickChart: React.FC<CandlestickChartProps> = ({
  ...
  const [currentHover, setCurrentHover] = useState<CandlestickData | null>(
    null,
  );
  ...
  const validateData = useCallback((candlestickData: CandlestickData[]) => {
    ...
    // 检查数据完整性
    const invalidCandles = candlestickData.filter(
      (candle) =>
        !candle.timestamp ||
      ...
      const volumeData = data.map((candle) => ({
        ...
        color:
          candle.close >= candle.open
            ? config.colors.bullish
            : config.colors.bearish,
      }));
  ...
  const createChart = useCallback(async () => {
    ...
    try {
      ...
      const { createChart, IChartApi, ISeriesApi } = await import(
        'lightweight-charts'
      );
      ...
      // 创建图表配置
      const chartConfig = createLightweightChartConfig(
        config,
        chartWidth,
        chartHeight,
      );
          ...
          <div className="text-sm space-y-1">
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
              ...
              <div className="flex justify-between">
                ...
                <span
                  className={
                    hoverStats.isBullish ? 'text-green-600' : 'text-red-600'
                  }
                >
              ...
              <div className="flex justify-between">
                ...
                <span className="text-gray-800">
                  {hoverStats.high.toFixed(2)}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">最低:</span>
                <span className="text-gray-800">
                  {hoverStats.low.toFixed(2)}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">成交量:</span>
                <span className="text-gray-800">
                  {hoverStats.volume.toLocaleString()}
                </span>
              </div>
              <div className="flex justify-between">
                ...
                >
                  {hoverStats.priceChange >= 0 ? '+' : ''}
                  {hoverStats.priceChange.toFixed(2)}(
                  {hoverStats.priceChangePercent >= 0 ? '+' : ''}
                  {hoverStats.priceChangePercent.toFixed(2)}%)
                </span>
              </div>
            </div>
          </div>
