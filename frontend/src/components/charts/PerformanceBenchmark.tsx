...
export const PerformanceBenchmark: React.FC<PerformanceBenchmarkProps> = ({
  ...
  const generateTestData = useCallback(
    async (size: number): Promise<CandlestickData[]> => {
  ...
  const testLightweightCharts = useCallback(
    async (data: CandlestickData[]): Promise<number> => {
  ...
  // 计算性能提升倍数
  const getPerformanceImprovement = useCallback(
    (lwTime: number, chartjsTime: number): number => {
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <p className="text-2xl font-bold text-blue-600">
                  {formatRenderTime(
                    results
                      ...
                      results.filter(
                        (r) => r.chartLibrary === 'Lightweight Charts',
                      ).length,
                  )}
                </p>
              </div>
              ...
              <div className="bg-green-50 p-4 rounded-lg">
                ...
                <p className="text-2xl font-bold text-green-600">
                  {(() => {
                    const lwResults = results.filter(
                      (r) => r.chartLibrary === 'Lightweight Charts',
                    );
                    const chartjsResults = results.filter(
                      (r) => r.chartLibrary === 'Chart.js',
                    );

                    if (lwResults.length === 0 || chartjsResults.length === 0)
                      return 'N/A';

                    const avgLw =
                      lwResults.reduce((sum, r) => sum + r.renderTime, 0) /
                      lwResults.length;
                    const avgChartjs =
                      chartjsResults.reduce((sum, r) => sum + r.renderTime, 0) /
                      chartjsResults.length;
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                  ...
                  <div className="space-y-2">
                    {testSizes.map((size) => {
                      const lwResult = results.find(
                        (r) =>
                          r.chartLibrary === 'Lightweight Charts' &&
                          r.dataPoints === size,
                      );
                      const chartjsResult = results.find(
                        (r) =>
                          r.chartLibrary === 'Chart.js' &&
                          r.dataPoints === size,
                      );
                    ...
                    {testSizes.map((size) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span
                            className={`text-sm font-medium ${colors[performance]}`}
                          >
                            {labels[performance]}
                          </span>
                        </div>
