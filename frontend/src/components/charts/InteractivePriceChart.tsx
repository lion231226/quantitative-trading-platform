...
import { ChartInteractions, createChartInteractions } from '@/utils/chartInteractions';
...
const InteractivePriceChart = React.forwardRef<any, InteractivePriceChartProps>(({
  ...
  height = 400,
  width,
}: InteractivePriceChartProps, ref) => {
  ...
  const [chartInteractions, setChartInteractions] = useState<ChartInteractions | null>(null);
  const [chartExporter, setChartExporter] = useState<ChartExporter | null>(null);
  ...
  const optimizedData = useMemo(() => {
    ...
    return {
      ...
      movingAverages: sampleData(data.movingAverages, perfConfig.maxDataPoints),
    };
  }, [data, perfConfig]);
  ...
  const chartConfig = useMemo(() => {
    const config = createChartConfig(optimizedData, {
      animation: {
        duration: perfConfig.enableAnimation ? perfConfig.animationDuration : 0,
      },
      interaction: {
        mode: 'index',
        intersect: false,
  ...
  const handleSignalClick = useCallback(
    (event: any, elements: any[]) => {
      if (elements.length > 0) {
        ...
        const nearestSignal = findNearestSignal(optimizedData.signals, targetDate);
        if (nearestSignal) {
    ...
    return (
      <div className="flex items-center justify-between p-4 bg-gray-50 border-b">
        ...
        <div className="flex items-center space-x-2">
          ...
          <Button variant="outline" size="sm" onClick={() => handleExport('png')} disabled={isLoading}>
            PNG
          </Button>
...
}) as React.ForwardRefExoticComponent<InteractivePriceChartProps & React.RefAttributes<any>>;
