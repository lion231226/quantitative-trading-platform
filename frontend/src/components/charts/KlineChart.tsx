...
export const KlineChart: React.FC<KlineChartProps> = ({
  ...
  const createChart = useCallback(async () => {
    ...
    try {
      ...
      // 设置K线数据
      const chartData = convertToLightweightChartsData(
        optimizedData.candlesticks,
      );
      ...
      if (onSignalClick) {
        chart.subscribeClick((param: any) => {
          if (param.time && param.seriesData.size > 0) {
            ...
            const signals = optimizedData.signals?.filter(
              (s) => s.timestamp === time,
            );
  ...
  const updateChart = useCallback(() => {
    if (!chartRef.current || !candlestickSeriesRef.current || !optimizedData)
      return;
    ...
    <div className={`kline-chart-container ${className}`}>
      ...
      {dataStats && (
        <div className="mb-2 flex justify-between text-xs text-gray-500">
          ...
          <span>
            日期范围: {dataStats.dateRange.start} 至 {dataStats.dateRange.end}
          </span>
          {dataStats.warnings > 0 && (
            <span className="text-yellow-600">
              ⚠️ {dataStats.warnings} 个警告
            </span>
          )}
        </div>
      )}
      ...
      {config.performance.enableDataSampling && (
        <div className="mt-2 text-xs text-gray-500 text-center">
          {optimizedData.candlesticks.length < data.candlesticks.length && (
            <span>
              💡 数据采样优化: {optimizedData.candlesticks.length} /{' '}
              {data.candlesticks.length} 数据点
            </span>
          )}
        </div>
      )}
