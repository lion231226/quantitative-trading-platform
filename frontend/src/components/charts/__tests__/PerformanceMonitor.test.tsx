...
describe('PerformanceMonitor', () => {
  ...
  test('shows optimization suggestions based on performance', () => {
    ...
    expect(screen.getByText(/建议减少数据点数量或启用数据采样/)).toBeInTheDocument()
    expect(screen.getByText(/大数据量场景，建议启用智能采样/)).toBeInTheDocument()
  })
  ...
  test('displays correct performance level text', () => {
    ...
    const { rerender } = render(<PerformanceMonitor {...excellentProps} showDetails={true} />)
    expect(screen.getByText('优秀')).toBeInTheDocument()

    // 良好性能
    rerender(<PerformanceMonitor {...excellentProps} fps={40} showDetails={true} />)
    expect(screen.getByText('良好')).toBeInTheDocument()

    // 一般性能
    rerender(<PerformanceMonitor {...excellentProps} fps={20} showDetails={true} />)
    expect(screen.getByText('一般')).toBeInTheDocument()

    // 需要优化
    rerender(<PerformanceMonitor {...excellentProps} fps={8} showDetails={true} />)
