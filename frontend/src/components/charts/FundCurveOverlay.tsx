...
import { FundCurveData, DualYAxisConfig, PerformanceMetrics } from '../../types/kline.types'
...
const FundCurveOverlay: React.FC<FundCurveOverlayProps> = ({
  ...
  // 计算性能指标
  const calculateMetrics = useCallback((data: FundCurveData['data']): PerformanceMetrics => {
    if (data.length < 2) {
      return {
        returnRate: 0,
        maxDrawdown: 0,
  ...
  useEffect(() => {
    ...
    fundCurves.forEach((fundCurve) => {
      ...
      const series = chart.addLineSeries({
        ...
        lineStyle: fundCurve.lineType === 'dashed' ? LineStyle.Dashed :
                   fundCurve.lineType === 'dotted' ? LineStyle.Dotted : LineStyle.Solid,
  ...
  }, [fundCurves, dualYAxisConfig, calculateMetrics, onCurveClick, onMetricsUpdate])
