...
import { StrategyTypeConfig, StrategyType } from '@/types/strategy';
import {
  STRATEGY_TYPES,
        ...
        {strategy.status !== 'available' && (
          <CardContent className="pt-0">
            <div className="text-xs text-gray-500 bg-gray-50 p-2 rounded">
              {strategy.status === 'coming-soon' && '此策略正在开发中，敬请期待！'}
              {strategy.status === 'experimental' && '此策略为实验性功能，建议谨慎使用。'}
            </div>
          </CardContent>
        )}
      </Card>
    ...
    <div className={className}>
      ...
      {viewMode === 'all' ? (
        ...
        <div className="space-y-6">
          {Object.entries(categorizedStrategies).map(([categoryKey, strategies]) => (
            <div key={categoryKey}>
              <div className="flex items-center gap-2 mb-3">
                <Badge
                  variant="outline"
                  className={STRATEGY_CATEGORY_LABELS[categoryKey as keyof typeof STRATEGY_CATEGORY_LABELS].color}
                >
      ...
      <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
        <div className="flex items-start gap-2">
          ...
          <div className="flex-1">
            <h4 className="text-sm font-medium text-blue-900 mb-1">策略选择建议</h4>
            <ul className="text-xs text-blue-800 space-y-1">
              <li>• <strong>初学者</strong>：建议从单均线策略开始，简单易懂</li>
              <li>• <strong>进阶用户</strong>：可尝试双均线策略，信号更稳定</li>
              <li>• <strong>专业用户</strong>：RSI和MACD策略适合震荡市场</li>
              <li>• <strong>实验性功能</strong>：请在充分理解风险后使用</li>
            </ul>
          </div>
        </div>
      </div>
