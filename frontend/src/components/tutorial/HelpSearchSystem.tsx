...
import { Search, Filter, Clock, TrendingUp, Star, X, ChevronDown } from 'lucide-react';
...
const HelpSearchSystem: React.FC<HelpSearchSystemProps> = ({
  ...
  const filteredAndSortedResults = React.useMemo(() => {
    ...
    if (selectedCategory !== 'all') {
      results = results.filter(result => result.content.category === selectedCategory);
    }

    // 难度过滤
    if (selectedDifficulty !== 'all') {
      results = results.filter(result => result.content.difficulty === selectedDifficulty);
    }
  ...
  const renderSearchResult = (result: SearchResult) => (
    ...
    >
      <div className="flex items-start justify-between mb-2">
        <h4 className="font-medium text-gray-900 text-sm flex-1">{result.content.title}</h4>
        <div className="flex items-center gap-2 ml-2">
          <span className={`px-2 py-1 text-xs rounded-full ${
            result.content.category === 'concept' ? 'bg-blue-100 text-blue-700' :
            result.content.category === 'feature' ? 'bg-green-100 text-green-700' :
            result.content.category === 'troubleshooting' ? 'bg-red-100 text-red-700' :
            'bg-purple-100 text-purple-700'
          }`}>
            {result.content.category === 'concept' ? '概念' :
             result.content.category === 'feature' ? '功能' :
             result.content.category === 'troubleshooting' ? '问题' :
             '实践'}
          </span>
          <span className={`px-2 py-1 text-xs rounded ${
            result.content.difficulty === 'beginner' ? 'bg-gray-100 text-gray-600' :
            result.content.difficulty === 'intermediate' ? 'bg-yellow-100 text-yellow-700' :
            'bg-red-100 text-red-700'
          }`}>
            {result.content.difficulty === 'beginner' ? '初级' :
             result.content.difficulty === 'intermediate' ? '中级' : '高级'}
          </span>
        </div>
      </div>
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <h4 className="font-medium text-gray-900 text-sm truncate">{content.title}</h4>
          <p className="text-gray-600 text-xs mt-1 line-clamp-2">{content.content}</p>
            ...
            <>
              {filteredAndSortedResults.length > 0 ? (
                ...
                <div className="p-8 text-center">
                  ...
                  <p className="text-gray-400 text-xs mt-1">尝试使用其他关键词或调整筛选条件</p>
                </div>
              )}
            </>
          )}
              ...
              {showSearchHistory && searchHistory.length > 0 && (
                <div>
                  <div className="px-4 py-2 bg-gray-50 border-b border-gray-200 flex items-center justify-between">
                    <span className="text-xs font-medium text-gray-700">最近搜索</span>
                    <button
              ...
              {showPopularContent && popularContent.length > 0 && (
                <div>
                  <div className="px-4 py-2 bg-yellow-50 border-b border-gray-200 flex items-center gap-2">
                    ...
                    <span className="text-xs font-medium text-yellow-700">热门内容</span>
                  </div>
                  {popularContent.map((content, index) => renderPopularContent(content, index))}
                </div>
              )}
            </div>
          )}
