...
import { HelpContent, UserContext } from '@/services/contextHelpService';
...
const FAQ_DATABASE: FAQItem[] = [
  {
    ...
    question: '什么是移动平均线？',
    answer:
      '移动平均线（Moving Average，MA）是技术分析中最常用的指标之一。它通过计算特定时间段内的平均价格来平滑价格波动，帮助识别趋势方向。常见的类型包括简单移动平均线（SMA）和指数移动平均线（EMA）。SMA给予所有价格相同权重，而EMA给予近期价格更高权重，因此对价格变化更敏感。',
    category: 'concepts',
    difficulty: 'beginner',
...
const FAQSystem: React.FC<FAQSystemProps> = ({
  ...
  const filteredFAQs = useMemo(() => {
    ...
    if (selectedDifficulty !== 'all') {
      filtered = filtered.filter(
        (faq) => faq.difficulty === selectedDifficulty,
      );
    }
  ...
  function calculateFAQRelevance(faq: FAQItem, context: UserContext): number {
    ...
    if (
      context.errorsEncountered.length > 0 &&
    ...
    // 基于用户行为匹配
    if (
      context.previousActions.some((action) =>
        faq.relatedTopics.some((topic) =>
          action.toLowerCase().includes(topic.toLowerCase()),
        ),
      )
    ) {
      score += 2;
  ...
  const renderFAQItem = (faq: FAQItem) => {
    ...
    const helpfulPercentage =
      faq.totalVotes > 0
        ? Math.round((faq.helpfulCount / faq.totalVotes) * 100)
        : 0;
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <span
                  className={`inline-flex items-center gap-1 px-2 py-1 text-xs rounded-full bg-${categoryConfig.color}-100 text-${categoryConfig.color}-700`}
                >
                  {categoryConfig.icon}
            ...
            {showStats && (
              <div className="mt-3 pt-3 border-t border-gray-200 flex items-center justify-between text-xs text-gray-500">
                <div className="flex items-center gap-3">
                  <span>
                    {faq.helpfulCount}/{faq.totalVotes} 人觉得有用
                  </span>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-2 text-xs text-gray-500">
                    ...
                    <span>
                      {Math.ceil(link.estimatedReadTime / 60)}分钟阅读
                    </span>
                  </div>
                </div>
              </div>
            ...
            {Object.entries(faqsByCategory).map(([category, faqs]) => {
              ...
              return (
                <div key={category} className="space-y-3">
                  ...
                  >
                    <span
                      className={`inline-flex items-center gap-1 px-3 py-1 text-sm font-medium rounded-lg bg-${categoryConfig.color}-100 text-${categoryConfig.color}-700`}
                    >
                      {categoryConfig.icon}
                      {categoryConfig.label}
                  ...
                  {isCategoryExpanded && (
                    <div className="space-y-3">{faqs.map(renderFAQItem)}</div>
                  )}
                </div>
              );
            })}
