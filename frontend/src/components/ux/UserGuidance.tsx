      ...
      <>
        ...
        >
          <Card className="p-6">
            ...
            {/* 内容 */}
            <div className="text-gray-700 mb-4">{step.content}</div>
...
export const UserGuideManager = memo<UserGuideManagerProps>(
  ({ guide, onStart, onComplete, onSkip, onStepChange }) => {
    ...
    const checkTriggerConditions = useCallback(() => {
      ...
      if (
        conditions.pagePath &&
        window.location.pathname !== conditions.pagePath
      ) {
        return false;
      }
...
export function UserProgressTracker({
  ...
  const updateProgress = useCallback(
    (
      ...
      if (
        action === 'complete' &&
        !updatedProgress.completedSteps.includes(`${guideId}_step_${stepIndex}`)
      ) {
      ...
      if (
        action === 'complete' &&
        stepIndex === updatedProgress.completedSteps.length - 1
      ) {
