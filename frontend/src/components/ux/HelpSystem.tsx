...
import { FAQItem, HelpDocument } from '@/types/ux.types';
...
export const HelpDocumentViewer = memo<HelpDocumentViewerProps>(
  ({ document, onRelatedDocumentClick }) => {
...
export const FAQItem = memo<FAQItemProps>(({ faq, onHelpfulClick }) => {
  const [isExpanded, setIsExpanded] = useState(false);
  ...
  return (
    <Card className="mb-4">
      <div className="p-4">
        ...
        {isExpanded && <div className="mb-4 text-gray-700">{faq.answer}</div>}
...
export const SearchComponent = memo<SearchComponentProps>(
  ({ onSearch, placeholder = '搜索帮助文档...', className }) => {
...
export function HelpCenter({
  ...
  const [selectedDocument, setSelectedDocument] = useState<HelpDocument | null>(
    null,
  );
  ...
  const { data: searchFAQs = [], isLoading: searchFAQsLoading } =
  ...
  const categories = useMemo(() => {
    const docCategories = Array.from(
    ...
    <div className={cn('inline-block', className)}>
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent className="max-w-md">
          ...
          <div className="text-sm text-gray-600">{helpContent.content}</div>
        </DialogContent>
      </Dialog>
    </div>
  );
