...
import { Plus, X, Edit2, Play, BarChart3, TrendingUp } from 'lucide-react'
import { ParameterGroup, StrategyParameters, StrategyResult, ColorConfig } from '@/types/parameter.types'
...
// 参数组颜色配置
const GROUP_COLORS = [
  { bg: 'bg-blue-100', border: 'border-blue-300', text: 'text-blue-800', chart: '#3B82F6' },
  { bg: 'bg-green-100', border: 'border-green-300', text: 'text-green-800', chart: '#10B981' },
  { bg: 'bg-purple-100', border: 'border-purple-300', text: 'text-purple-800', chart: '#8B5CF6' },
  { bg: 'bg-orange-100', border: 'border-orange-300', text: 'text-orange-800', chart: '#F97316' },
]
...
export const ParameterComparison: React.FC<ParameterComparisonProps> = ({
  ...
  // 更新参数组参数
  const updateGroupParameters = useCallback((groupId: string, parameters: StrategyParameters) => {
    setParameterGroups(prev =>
      prev.map(group =>
  ...
  // 运行单个参数组回测
  const runSingleBacktest = useCallback(async (group: ParameterGroup) => {
    setIsLoading(true)
    ...
    try {
      ...
      setParameterGroups(prev =>
        prev.map(g =>
          g.id === group.id ? { ...g, results: result } : g
        )
      )
    ...
    try {
      ...
      setParameterGroups(prev =>
        prev.map(group => {
          ...
          return groupResult ? { ...group, results: groupResult.result } : group
        })
      )
    } catch (error) {
          ...
          >
            ...
            <div className="space-y-3 mb-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => updateGroupParameters(group.id, {
                    ...group.parameters,
            ...
            {group.results ? (
              <div className={`p-3 rounded-lg ${colorConfig.bg}`}>
                <h5 className="text-xs font-semibold mb-2 text-gray-700">回测结果</h5>
      ...
      {parameterGroups.length > 1 && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          ...
          <p className="text-sm text-blue-700">
            您已添加 {parameterGroups.length} 组参数进行对比。点击"运行全部"可以并行运行所有组的回测，
