...
export function PercentageInput({
  ...
  const validateInput = useCallback(
    (input: string): number | null => {
      ...
      const precisioned =
        Math.round(stepped * Math.pow(10, precision)) / Math.pow(10, precision);
      ...
      <CardContent className="p-6">
        ...
        <div className="mt-6 space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            {COMMON_PRESETS.filter(
              (preset) => preset >= min && preset <= max,
            ).map((preset) => (
              <Button
                key={preset}
                variant={
                  Math.abs(value - preset) < 0.01 ? 'default' : 'outline'
                }
                size="sm"
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {Object.entries(RISK_CONFIG[type]).map(([level, config]) => {
                ...
                return (
                  <Card
                    key={level}
                    className={`p-3 ${isActive ? 'ring-2 ring-blue-500' : ''}`}
                  >
                    ...
                    <ul className="text-sm text-blue-800 space-y-1">
                      {type === 'stopLoss' ? (
                        <>
                          ...
                          <li>
                            • <strong>7-15%:</strong> 适合趋势跟踪，容忍较大波动
                          </li>
                          <li>
                            • <strong>建议:</strong>{' '}
                            根据个人风险承受能力和交易频率调整
                        ...
                        <>
                          <li>
                            • <strong>5-10%:</strong> 保守收益目标，稳定增长
                          </li>
                          <li>
                            • <strong>10-20%:</strong>{' '}
                            平衡收益预期，适合大多数市场
