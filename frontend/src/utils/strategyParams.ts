...
// 策略参数元数据配置
export const STRATEGY_PARAMETER_META: Record<
  keyof MovingAverageStrategyParams,
  StrategyParameterMeta
> = {
  ma_period: {
    type: 'number',
...
export const validateStrategyParams = (
  params: MovingAverageStrategyParams,
  ...
  // 验证信号确认参数
  if (
    params.min_cross_percentage < 0.0001 ||
    params.min_cross_percentage > 0.01
  ) {
...
export const safeGetValue = (
  params: Partial<MovingAverageStrategyParams>,
  key: keyof MovingAverageStrategyParams,
  defaultValue: any,
): any => {
...
export const getParametersByCategory = () => {
  ...
  const categorizedParams: Record<
    string,
    Array<{
      key: keyof MovingAverageStrategyParams;
