...
// 图案类型定义
export type PatternShape = 'triangle' | 'circle' | 'diamond' | 'square' | 'hexagon' | 'line'
...
export function createColorblindTexture(
  ...
  switch (pattern.shape) {
    ...
    case 'hexagon':
      ...
      break
    case 'line':
      drawLineTexture(ctx, pattern.symbol === '━' ? 'horizontal' : 'vertical', intensity)
      break
  }
 ...
 * 绘制三角形纹理
 */
function drawTriangleTexture(ctx: CanvasRenderingContext2D, intensity: number): void {
 ...
 * 绘制圆形纹理
 */
function drawCircleTexture(ctx: CanvasRenderingContext2D, intensity: number): void {
  ctx.strokeStyle = `rgba(0, 0, 0, ${intensity})`
  ctx.lineWidth = 1
 ...
 * 绘制菱形纹理
 */
function drawDiamondTexture(ctx: CanvasRenderingContext2D, intensity: number): void {
  ctx.strokeStyle = `rgba(0, 0, 0, ${intensity})`
  ctx.lineWidth = 1
 ...
 * 绘制方形纹理
 */
function drawSquareTexture(ctx: CanvasRenderingContext2D, intensity: number): void {
  ctx.strokeStyle = `rgba(0, 0, 0, ${intensity})`
  ctx.lineWidth = 1
 ...
 * 绘制六边形纹理
 */
function drawHexagonTexture(ctx: CanvasRenderingContext2D, intensity: number): void {
  ctx.strokeStyle = `rgba(0, 0, 0, ${intensity})`
  ctx.lineWidth = 1
 ...
 * 绘制线条纹理
 */
function drawLineTexture(ctx: CanvasRenderingContext2D, direction: 'horizontal' | 'vertical', intensity: number): void {
  ctx.strokeStyle = `rgba(0, 0, 0, ${intensity})`
  ctx.lineWidth = 1
...
export function validateColorblindConfig(config: ColorblindConfig): {
  ...
  // 验证模式
  const validModes: ColorblindMode[] = ['none', 'protanopia', 'deuteranopia', 'tritanopia', 'achromatopsia']
...
export function getColorblindCSSClass(
  ...
  return [baseClass, modeClass, patternClass, shapeClass].filter(Boolean).join(' ')
}