 ...
 * 创建默认K线图配置
 */
export const createDefaultKlineConfig = (overrides?: Partial<KlineConfig>): KlineConfig => {
  return {
    // 基础配置
    height: 400,
    timePeriod: TimePeriod.DAY_1,
 ...
 */
export const convertToLightweightChartsData = (klineData: CandlestickData[]) => {
 ...
 * 格式化时间显示
 */
export const formatTimeDisplay = (timestamp: number, timePeriod: TimePeriod): string => {
 ...
 * 格式化价格显示
 */
export const formatPriceDisplay = (price: number, decimals: number = 2): string => {
 ...
 * 获取价格变化颜色
 */
export const getPriceChangeColor = (change: number, config: KlineConfig): string => {
 ...
 * 检测数据完整性
 */
export const detectDataGaps = (data: CandlestickData[], period: TimePeriod): string[] => {
  const gaps: string[] = []
...
export class PerformanceBenchmarkUtil {
   ...
   * 创建基准测试结果
   */
  createResult(chartLibrary: 'Chart.js' | 'Lightweight Charts', dataPoints: number) {
    return {
      chartLibrary,
      dataPoints,
...
export class KlineDataSampler {
  ...
  private static intelligentSample(
    ...
    for (let i = step; i < data.length - step; i += step) {
      const chunk = data.slice(Math.max(0, i - step), Math.min(data.length, i + step))
   ...
   * 重要点判断标准：价格变化幅度大的点
   */
  private static findMostImportantPoint(chunk: CandlestickData[]): CandlestickData {
    if (chunk.length === 1) {
      return chunk[0]
    }
   ...
   * 计算采样统计信息
   */
  static getSamplingStats(originalData: CandlestickData[], sampledData: CandlestickData[]) {
    return {
      originalCount: originalData.length,
      sampledCount: sampledData.length,
      reductionRatio: ((originalData.length - sampledData.length) / originalData.length * 100).toFixed(1),
      preservationRate: (sampledData.length / originalData.length * 100).toFixed(1)
    }
  }
}