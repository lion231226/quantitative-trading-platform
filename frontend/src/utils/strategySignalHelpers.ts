...
export class StrategyValidator {
   ...
   * 验证策略参数的有效性
   */
  static validateParams(strategyId: string, params: StrategyParams): {
    isValid: boolean
    errors: string[]
    warnings: string[]
  } {
    ...
    try {
      ...
      errors.push(`参数验证过程中发生错误: ${error instanceof Error ? error.message : '未知错误'}`)
    }
   ...
   * 验证移动平均线交叉策略参数
   */
  private static validateMACrossParams(params: StrategyParams, errors: string[], warnings: string[]): void {
    const shortPeriod = params.shortPeriod
    const longPeriod = params.longPeriod

    if (!shortPeriod || typeof shortPeriod !== 'number' || shortPeriod < 1 || shortPeriod > 200) {
      errors.push('短期均线周期必须是1-200之间的数字')
    }

    if (!longPeriod || typeof longPeriod !== 'number' || longPeriod < 1 || longPeriod > 500) {
   ...
   * 验证RSI策略参数
   */
  private static validateRSIParams(params: StrategyParams, errors: string[], warnings: string[]): void {
   ...
   * 验证MACD策略参数
   */
  private static validateMACDParams(params: StrategyParams, errors: string[], warnings: string[]): void {
    const fastPeriod = params.fastPeriod
    const slowPeriod = params.slowPeriod
    const signalPeriod = params.signalPeriod

    if (!fastPeriod || typeof fastPeriod !== 'number' || fastPeriod < 2 || fastPeriod > 50) {
      errors.push('快线周期必须是2-50之间的数字')
    }

    if (!slowPeriod || typeof slowPeriod !== 'number' || slowPeriod < 10 || slowPeriod > 200) {
      errors.push('慢线周期必须是10-200之间的数字')
    }

    if (!signalPeriod || typeof signalPeriod !== 'number' || signalPeriod < 2 || signalPeriod > 50) {
   ...
   * 验证布林带策略参数
   */
  private static validateBollingerBandsParams(params: StrategyParams, errors: string[], warnings: string[]): void {
    const period = params.period
...
export class StrategyPerformanceAnalyzer {
   ...
   * 计算策略收益率
   */
  static calculateReturn(signals: StrategySignal[], initialCapital: number = 10000): {
    totalReturn: number
    returnRate: number
    maxDrawdown: number
    sharpeRatio: number
   ...
   * 模拟交易
   */
  private static simulateTrades(signals: StrategySignal[], initialCapital: number): Array<{
    entryPrice: number
    exitPrice: number
    profit: number
    entryTime: number
    ...
    for (const signal of signals.sort((a, b) => a.timestamp - b.timestamp)) {
      ...
      } else if (signal.signalType === 'sell' && position && position.type === 'buy') {
        // 平多仓
        trades.push({
   ...
   * 生成性能报告
   */
  static generatePerformanceReport(strategyId: string, signals: StrategySignal[], initialCapital: number = 10000): {
    strategyId: string
    totalSignals: number
    ...
    const recommendations = this.generateRecommendations(performance, signalsByType)
  ...
  private static generateRecommendations(
    ...
    // 基于信号频率的建议
    const totalSignals = Object.values(signalsByType).reduce((sum, count) => sum + count, 0)
    if (totalSignals > 100) {
...
export class StyleManager {
   ...
   * 生成策略特定样式
   */
  static generateStrategyStyle(strategyType: StrategyType, theme: 'light' | 'dark' = 'light'): Record<SignalType, SignalMarkerStyle> {
    const baseTheme = DEFAULT_SIGNAL_THEMES[theme]

    // 根据策略类型调整样式
    const strategySpecificStyles = this.getStrategySpecificStyles(strategyType, theme)
   ...
   * 获取策略特定样式
   */
  private static getStrategySpecificStyles(strategyType: StrategyType, theme: 'light' | 'dark'): Partial<Record<SignalType, SignalMarkerStyle>> {
   ...
   * 根据置信度调整样式
   */
  static adjustStyleByConfidence(baseStyle: SignalMarkerStyle, confidence: number): SignalMarkerStyle {
   ...
   * 获取信号标签文本
   */
  static getSignalLabelText(signal: StrategySignal, includeStrategy: boolean = false): string {
   ...
   * 生成对比样式
   */
  static generateComparisonStyle(baseStyle: SignalMarkerStyle, index: number, total: number): SignalMarkerStyle {
...
export class TimeUtils {
   ...
   * 格式化时间戳
   */
  static formatTimestamp(timestamp: number, format: 'date' | 'datetime' | 'time' = 'datetime'): string {
   ...
   * 计算时间间隔
   */
  static getTimeInterval(startTime: number, endTime: number): {
    days: number
    hours: number
    minutes: number
    totalHours: number
