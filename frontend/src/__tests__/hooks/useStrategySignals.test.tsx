...
const mockKlineDataService = klineDataService as jest.Mocked<typeof klineDataService>
...
describe('useStrategySignals', () => {
  ...
  const createWrapper = (config = {}) => {
    ...
    return renderHook(() => useStrategySignals({ ...defaultConfig, ...config }))
  }
  ...
  it('应该激活策略', async () => {
    ...
    await act(async () => {
      await result.current.activateStrategy('sma_crossover', { shortPeriod: 10, longPeriod: 30 })
    })

    expect(mockKlineDataService.activateStrategy).toHaveBeenCalledWith('sma_crossover', { shortPeriod: 10, longPeriod: 30 })
  ...
  it('应该处理激活策略错误', async () => {
    ...
    mockKlineDataService.activateStrategy.mockRejectedValue(new Error(errorMessage))
  ...
  it('应该停用策略', async () => {
    ...
    expect(mockKlineDataService.deactivateStrategy).toHaveBeenCalledWith('sma_crossover')
    expect(mockSignalRenderer.removeMarkers).toHaveBeenCalledWith('test-chart', ['sma_crossover'])
  ...
  it('应该更新策略参数', async () => {
    ...
    expect(mockKlineDataService.updateStrategyParams).toHaveBeenCalledWith('sma_crossover', newParams, 'TEST', '1d')
    expect(result.current.activeStrategies.get('sma_crossover')).toEqual(newParams)
  })
  ...
  it('应该刷新信号', async () => {
    ...
    expect(mockKlineDataService.getDataWithSignals).toHaveBeenCalledWith('TEST', '1d', undefined, true)
    expect(result.current.loading).toBe(false)
  })
  ...
  it('应该过滤信号', () => {
    ...
    act(() => {
      // 模拟设置信号数据（通常通过激活策略获得）
      result.current.signals = new Map([['sma_crossover', mockStrategySignalResult]])
    })
    ...
    expect(filteredArray.every(signal => signal.signalType === 'buy')).toBe(true)
  })
  ...
  it('应该获取最近信号', () => {
    ...
    expect(recentSignals[0].timestamp).toBeGreaterThanOrEqual(mockStrategySignals[1].timestamp)
  })
  ...
  it('应该获取统计信息', () => {
    ...
    expect(mockKlineDataService.getSignalStatistics).toHaveBeenCalledWith('sma_crossover')
  ...
  it('应该比较策略', () => {
    ...
    const comparison = result.current.compareStrategies('sma_crossover', 'rsi_oversold')

    expect(mockKlineDataService.compareStrategies).toHaveBeenCalledWith('sma_crossover', 'rsi_oversold')
  ...
  it('应该获取性能指标', () => {
    ...
    mockKlineDataService.getStrategyPerformance.mockReturnValue(mockPerformance)

    const performance = result.current.getPerformance('sma_crossover')

    expect(mockKlineDataService.getStrategyPerformance).toHaveBeenCalledWith('sma_crossover')
  ...
  it('应该自动更新信号', async () => {
    ...
    const initialCallCount = mockKlineDataService.getDataWithSignals.mock.calls.length

    // 等待自动更新
    await waitFor(() => {
      expect(mockKlineDataService.getDataWithSignals.mock.calls.length).toBeGreaterThan(initialCallCount)
    }, { timeout: 200 })
  ...
  describe('错误处理', () => {
    it('应该处理获取数据失败', async () => {
      ...
      mockKlineDataService.getDataWithSignals.mockRejectedValue(new Error(errorMessage))
    ...
    it('应该处理更新不存在的策略参数', async () => {
      ...
      const resultUpdate = await act(async () => {
        return await result.current.updateStrategyParams('nonexistent_strategy', { test: 'value' })
      })
  ...
  describe('边界情况', () => {
    it('应该处理空信号列表', () => {
      ...
      const filteredSignals = result.current.filterSignals({ signalTypes: ['buy'] })
    ...
    it('应该处理无效的过滤条件', () => {
      ...
      const filtered3 = result.current.filterSignals({ signalTypes: ['invalid_type' as any] })
