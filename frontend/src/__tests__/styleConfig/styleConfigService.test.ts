...
describe('StyleConfigService', () => {
  ...
  describe('getSignalStyle', () => {
    ...
    it('应该根据置信度调整样式', () => {
      const highConfidenceStyle = service.getSignalStyle('sma_crossover', 'buy', 90)
      const lowConfidenceStyle = service.getSignalStyle('sma_crossover', 'buy', 30)

      expect(highConfidenceStyle.opacity).toBeGreaterThan(lowConfidenceStyle.opacity)
      expect(highConfidenceStyle.size).toBeGreaterThanOrEqual(lowConfidenceStyle.size)
    })
  ...
  describe('样式预设', () => {
    ...
    it('应该创建样式预设', () => {
      // 先添加一些用户配置
      service.updateUserStyleConfig('test_strategy1', 'buy', { color: '#ff0000', size: 15 })
      service.updateUserStyleConfig('test_strategy1', 'sell', { color: '#00ff00', size: 15 })
      ...
      const preset = service.getAllStylePresets().find(p => p.id === presetId)
    ...
    it('应该应用样式预设', () => {
      // 创建一个预设
      service.updateUserStyleConfig('test_strategy', 'buy', { color: '#ff00ff', size: 20 })
      const presetId = service.createStylePreset('Test Preset', 'Description', 'custom')
    ...
    it('应该删除样式预设', () => {
      const presetId = service.createStylePreset('Test', 'Description', 'custom')

      expect(service.getAllStylePresets().some(p => p.id === presetId)).toBe(true)

      service.removeStylePreset(presetId)

      expect(service.getAllStylePresets().some(p => p.id === presetId)).toBe(false)
    })
  })
  ...
  describe('导入导出', () => {
    it('应该导出样式配置', () => {
      // 设置一些配置
      service.setCurrentTheme('dark')
      service.updateUserStyleConfig('test_strategy', 'buy', { color: '#ff0000' })
      const presetId = service.createStylePreset('Test', 'Description', 'custom')
    ...
    it('应该导入样式配置', () => {
      ...
      expect(service.getCurrentTheme()?.id).toBe('dark')
      const importedStyle = service.getUserStyleConfig('imported_strategy', 'buy')
  ...
  describe('重置功能', () => {
    it('应该重置为默认样式', () => {
      ...
      expect(service.getAllStylePresets().filter(p => p.category === 'custom')).toHaveLength(0)
    })
  })

  describe('样式优先级', () => {
    it('用户样式应该覆盖预设样式', () => {
      service.updateUserStyleConfig('sma_crossover', 'buy', { color: '#ff0000' })
