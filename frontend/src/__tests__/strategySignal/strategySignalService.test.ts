...
describe('StrategySignalManager', () => {
  ...
  describe('loadSignals', () => {
    it('应该成功加载SMA金叉死叉策略信号', async () => {
      ...
      const result = await signalManager.loadSignals(
        strategyId,
        params,
        symbol,
        period,
    ...
    it('应该正确计算RSI超卖策略信号', async () => {
      ...
      expect(result.strategyId).toBe(strategyId);
      expect(
        result.signals.every((signal) => signal.signalType === 'buy'),
      ).toBe(true);
      expect(
        result.signals.every(
          (signal) => signal.strategyType === 'rsi_oversold',
        ),
      ).toBe(true);
    });
    ...
    it('应该缓存计算结果', async () => {
      ...
      const result1 = await signalManager.loadSignals(
        strategyId,
        params,
        symbol,
        period,
      ...
      const result2 = await signalManager.loadSignals(
        strategyId,
        params,
        symbol,
        period,
  ...
  describe('calculateDifference', () => {
    it('应该正确计算信号差异', () => {
      const oldSignals = [
        MockDataGenerator.generateStrategySignal(
          '1',
          Date.now(),
          100,
          'buy',
      ...
      const difference = signalManager.calculateDifference(
        oldSignals,
        newSignals,
      );
    ...
    it('应该检测到修改的信号', () => {
      const baseSignal = MockDataGenerator.generateStrategySignal(
        '1',
        Date.now(),
        100,
        'buy',
  ...
  describe('optimizeSignals', () => {
    ...
    it('应该移除重复信号', () => {
      const duplicateSignal = MockDataGenerator.generateStrategySignal(
        '1',
        Date.now(),
        100,
        'buy',
  ...
  describe('compareSignals', () => {
    it('应该正确比较两组信号', () => {
      const commonSignal = MockDataGenerator.generateStrategySignal(
        '1',
        Date.now(),
        100,
        'buy',
  ...
  describe('缓存管理', () => {
    it('应该正确管理缓存大小限制', async () => {
      ...
      // 添加超过限制的缓存项
      await signalManager.loadSignals(
        'sma_crossover',
      ...
      // 最旧的缓存项应该被移除
      const oldCacheKey = signalManager['generateCacheKey'](
        'sma_crossover',
        { shortPeriod: 5, longPeriod: 10 },
        'TEST',
        '1d',
    ...
    it('应该正确清理过期缓存', async () => {
      ...
      const cacheKey = signalManager['generateCacheKey'](
        'sma_crossover',
