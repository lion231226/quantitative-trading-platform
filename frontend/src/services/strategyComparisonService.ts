...
export class StrategyComparisonService {
  ...
  async createComparison(
    ...
    // 基础对比分析
    const basicComparison = strategySignalManager.compareSignals(
      filteredSignalsA,
      filteredSignalsB,
    );
    ...
    for (let i = 0; i < strategies.length; i++) {
      for (let j = i + 1; j < strategies.length; j++) {
        ...
        try {
          ...
          console.error(
            `批量对比失败 ${strategyA.id} vs ${strategyB.id}:`,
            error,
          );
        }
      }
  ...
  private analyzeCorrelation(
    ...
    const sellSignal = this.calculateSignalCorrelation(
      sellSignalsA,
      sellSignalsB,
    );
  ...
  private analyzePerformance(
    ...
    const winRates = {
      ...
      winner:
        returnsA.winRate > returnsB.winRate
          ? ('A' as const)
          : returnsA.winRate < returnsB.winRate
            ? ('B' as const)
            : ('tie' as const),
  ...
  private analyzeTiming(
    ...
    for (const timestamp of allTimestamps) {
      ...
      if (signalA && !signalB) {
        ...
        signalA &&
        signalB &&
        signalA.signalType !== signalB.signalType
      ) {
        // 检查是否为相反信号（买入vs卖出）
        if (
          (signalA.signalType === 'buy' && signalB.signalType === 'sell') ||
  ...
  private generateRecommendations(
    ...
    if (performance.totalReturns.difference > 10) {
      recommendations.push(
        `策略${performance.totalReturns.winner}收益表现显著优于另一个策略`,
      );
    }
    ...
    // 基于风险的建议
    if (
      risk.signalVolatility.strategyA >
      risk.signalVolatility.strategyB * 1.5
    ) {
  ...
  private filterSignals(
    ...
    if (config.signalTypes && config.signalTypes.length > 0) {
      filtered = filtered.filter((s) =>
        config.signalTypes!.includes(s.signalType),
      );
    }
   ...
   * 私有方法：计算平均领先时间
   */
  private calculateAverageLeadTime(
    signalsA: StrategySignal[],
    signalsB: StrategySignal[],
  ): number {
    ...
    for (const signalA of signalsA) {
      const closestB = signalsB.find(
        (s) => Math.abs(s.timestamp - signalA.timestamp) < 60000, // 1分钟内
      );
  ...
  private calculateSignalVolatility(signals: StrategySignal[]): number {
    ...
    const intervals = this.calculateIntervals(
      signals.map((s) => s.timestamp).sort(),
    );
    const mean =
