import { CandlestickData, KlineData, TimePeriod } from '../types/kline.types';
...
export class TimePeriodDataManager {
  ...
  private listeners: Map<TimePeriod, ((data: CandlestickData[]) => void)[]> =
    new Map();
  ...
  async updateLatestData(
    ...
    if (existingData.length > 0) {
      ...
      if (
        ...
        new Date(lastData.timestamp).getTime()
      ) {
        // 添加新数据点
  ...
  recommendPeriodForDateRange(startDate: string, endDate: string): TimePeriod {
    // 首先基于时间范围推荐
    const recommended = timePeriodService.recommendTimePeriod(
      startDate,
      endDate,
    );
...
// 导出工厂函数
export function createTimePeriodDataManager(
  symbol: string,
): TimePeriodDataManager {
