...
export class AdaptivePerformanceService {
  ...
  getPerformanceTrend(): {
    ...
    if (olderMetrics.length > 0) {
      const olderAvgFps =
        olderMetrics.reduce((sum, m) => sum + m.fps, 0) / olderMetrics.length;
      const olderAvgRenderTime =
        olderMetrics.reduce((sum, m) => sum + m.renderTime, 0) /
        olderMetrics.length;
