...
export class TimePeriodService {
  ...
  private static interpolateToFinerPeriod(
    ...
    console.warn(
      `无法从 ${fromPeriod} 插值到 ${toPeriod}，建议直接从API获取数据`,
    );
    return data;
  }
  ...
  static validateTimePeriodConsistency(data: CandlestickData[]): boolean {
    ...
    const inconsistentCount = intervals.filter(
      (interval) => Math.abs(interval - avgInterval) / avgInterval > 0.5,
    ).length;
  ...
  static fillMissingData(
    ...
    while (currentTimestamp <= end) {
      ...
      while (
        ...
        new Date(data[dataIndex].timestamp).getTime() < currentTimestamp
      ) {
