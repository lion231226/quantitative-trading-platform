...
export interface InteractionListener {
  ...
  onDataPointHover?: (data: CandlestickData | null, position: { x: number; y: number }) => void
...
export class ChartInteractionService {
   ...
   * 通知所有监听器
   */
  private notifyListeners(event: keyof InteractionListener, ...args: any[]): void {
  ...
  private setupChartInteractions(): void {
    ...
    // 添加事件监听器
    this.chartInstance.subscribeCrosshairMove(this.handleCrosshairMove.bind(this))
  ...
  private handleClick(param: any): void {
    ...
    const candlestickData = this.findCandlestickData(param.time)
    if (candlestickData) {
      this.notifyListeners('onDataPointClick', candlestickData, param.originalEvent as MouseEvent)
    }
  }
  ...
  private setupKeyboardShortcuts(): void {
    ...
    // 显示控制
    keyboardShortcutService.addListener('toggleCrosshair', () => this.toggleCrosshair())
    ...
    // 其他功能
    keyboardShortcutService.addListener('exportChart', () => this.exportChart('png'))
    keyboardShortcutService.addListener('fullscreen', () => this.toggleFullscreen())
    keyboardShortcutService.addListener('togglePerformance', () => this.togglePerformance())
  }
  ...
  zoomIn(center?: number): void {
    ...
    if (visibleRange) {
      ...
      const zoomCenter = center !== undefined ? center : (visibleRange.from + visibleRange.to) / 2
   ...
   */
  nextPeriod(): void {
    const periods = ['1m', '5m', '15m', '30m', '1h', '4h', '1d', '1w', '1M'] as TimePeriod[]
  ...
  toggleFullscreen(): void {
    const container = document.querySelector('.kline-chart-container') as HTMLElement
