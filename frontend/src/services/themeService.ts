...
export class ThemeService {
  ...
  validateTheme(theme: ThemeConfig): ThemeValidationResult {
    ...
    const requiredColors: (keyof MarketColors)[] = [
      'bullish', 'bearish', 'volume', 'grid', 'background', 'text', 'border'
    ]
  ...
  importThemes(exportData: ThemeExportFormat): void {
    ...
    if (exportData.userPreferences) {
      const { currentTheme, marketMode, colorblindConfig, customColors } = exportData.userPreferences
   ...
   * 获取市场颜色
   */
  private getMarketColors(marketMode: MarketMode, themeMode: ThemeMode): MarketColors {
   ...
   * 应用色盲颜色
   */
  private applyColorblindColors(colorblindMode: ColorblindMode, marketMode: MarketMode): MarketColors {
    const baseColors = DEFAULT_MARKET_COLORS[marketMode]
