...
export class LightweightChartSignalRenderer implements ISignalRenderer {
  ...
  async addMarkers(chartId: string, signals: StrategySignal[]): Promise<void> {
    ...
    if (markers.length + signals.length > this.config.maxMarkers) {
      console.warn(`标记点数量超过限制 ${this.config.maxMarkers}，将移除最旧的标记点`)
  ...
  async removeMarkers(chartId: string, signalIds: string[]): Promise<void> {
    ...
    if (markerIds.length > 0) {
      ...
      // 更新内部状态
      const updatedMarkers = markers.filter(m => !signalIds.includes(m.signalId))
   ...
   * 更新标记点
   */
  async updateMarkers(chartId: string, signals: StrategySignal[]): Promise<void> {
    const chart = this.charts.get(chartId)
    if (!chart) {
      return
    }
   ...
   * 动画处理标记点过渡
   */
  async animateMarkerTransition(chartId: string, diff: SignalDifference): Promise<void> {
    if (!this.config.animationEnabled) {
      // 如果动画禁用，直接执行更新
      await this.updateMarkers(chartId, diff.added)
      await this.removeMarkers(chartId, diff.removed.map(s => s.id))
      return
    }
   ...
   * 批量更新标记点
   */
  async batchUpdateMarkers(chartId: string, operations: MarkerOperation[]): Promise<void> {
    const chart = this.charts.get(chartId)
    if (!chart) {
      return
    }
   ...
   * 私有方法：创建标记点配置
   */
  private createMarkerConfig(signal: StrategySignal, style: MarkerStyle): MarkerConfig {
   ...
   * 私有方法：获取标记点位置
   */
  private getMarkerPosition(signalType: string): 'aboveBar' | 'belowBar' | 'inBar' {
    switch (signalType) {
      case 'buy':
      case 'take_profit':
        return 'aboveBar'
   ...
   * 私有方法：映射信号形状到标记点形状
   */
  private mapSignalShapeToMarkerShape(shape: MarkerShape): MarkerConfig['shape'] {
   ...
   * 私有方法：添加标记点批次
   */
  private async addMarkerBatch(chartId: string, signals: StrategySignal[]): Promise<void> {
    const chart = this.charts.get(chartId)
    const markers = this.markers.get(chartId) || []
   ...
   * 私有方法：检查信号是否有显著变化
   */
  private hasSignificantChange(oldSignal: StrategySignal, newSignal: StrategySignal): boolean {
    return (
   ...
   * 私有方法：标记点添加动画
   */
  private async animateMarkerAdd(chartId: string, signals: StrategySignal[]): Promise<void> {
    const markers = this.markers.get(chartId) || []
    ...
    // 使用CSS动画或自定义动画逻辑
    await this.performAnimation(chartId, 'fadeIn', signals.map(s => s.id))
   ...
   * 私有方法：标记点移除动画
   */
  private async animateMarkerRemove(chartId: string, signals: StrategySignal[]): Promise<void> {
    // 执行淡出动画
    await this.performAnimation(chartId, 'fadeOut', signals.map(s => s.id))

    // 动画完成后移除标记点
    await this.removeMarkers(chartId, signals.map(s => s.id))
  }

  /**
   * 私有方法：标记点更新动画
   */
  private async animateMarkerUpdate(chartId: string, modifications: Array<{ old: StrategySignal; new: StrategySignal }>): Promise<void> {
    ...
    // 动画完成后更新标记点
    await this.updateMarkers(chartId, modifications.map(m => m.new))
  }

  /**
   * 私有方法：执行动画
   */
  private async performAnimation(chartId: string, animationType: 'fadeIn' | 'fadeOut' | 'transform', signalIds: string[]): Promise<void> {
   ...
   * 私有方法：触发标记点动画
   */
  private triggerMarkerAnimation(chartId: string, signalIds: string[], animationConfig: any): void {
    // 这里可以实现具体的DOM操作来触发动画
