...
export class KlineDataValidator {
  ...
  static validate(data: KlineData): DataValidationResult {
    ...
    const dateRange = {
      ...
      end: candlesticks.length > 0 ? candlesticks[candlesticks.length - 1].timestamp : ''
    }
...
export class KlineDataSampler {
   ...
   * 保持重要点的采样
   */
  private static preserveImportantPoints(data: CandlestickData[], maxPoints: number): CandlestickData[] {
    // 始终保留第一个和最后一个数据点
    ...
    if (importantPoints.length < remainingPoints) {
      const uniformStep = Math.floor(data.length / (remainingPoints - importantPoints.length))
    ...
    // 合并所有选中的点并排序
    const allPoints = [...result, ...selected].sort((a, b) =>
   ...
   * 均匀采样
   */
  private static uniformSampling(data: CandlestickData[], maxPoints: number): CandlestickData[] {
   ...
   * 选择最重要的点
   */
  private static selectTopPoints(points: CandlestickData[], count: number): CandlestickData[] {
...
export class KlineDataService {
   ...
   * 从API获取数据（示例实现）
   */
  private async fetchFromAPI(symbol: string, period: TimePeriod): Promise<KlineData | null> {
    // TODO: 实现真实的API调用
    // 这里返回模拟数据作为示例
    return {
   ...
   * 激活策略
   */
  async activateStrategy(strategyId: string, params: StrategyParams): Promise<void> {
  ...
  async getDataWithSignals(
    ...
    fromCache: boolean = true
  ): Promise<{ data: KlineData; signals: Map<string, StrategySignalResult> } | null> {
    // 获取K线数据
    ...
    for (const [strategyId, params] of this.activeStrategies) {
      ...
      try {
        const signalResult = await strategySignalManager.loadSignals(strategyId, params, symbol, period, data)
  ...
  async updateStrategyParams(
    ...
    const newResult = await strategySignalManager.loadSignals(strategyId, newParams, symbol, period, newData)
    this.lastSignalResults.set(strategyId, newResult)

    // 计算差异
    return strategySignalManager.calculateDifference(oldResult.signals, newResult.signals)
  }

  /**
   * 获取策略信号差异
   */
  async getSignalDifference(strategyId: string, symbol: string, period: TimePeriod): Promise<SignalDifference | null> {
    const oldResult = this.lastSignalResults.get(strategyId)
    if (!oldResult) {
      return null
    }
    ...
    const newResult = await strategySignalManager.loadSignals(strategyId, params, symbol, period, newData)
    this.lastSignalResults.set(strategyId, newResult)
  ...
  getSignalStatistics(strategyId?: string): Map<string, any> {
    ...
    if (strategyId) {
      ...
      const result = this.lastSignalResults.get(strategyId)
      if (result) {
        statistics.set(strategyId, strategySignalManager.getStatistics(result.signals))
      }
    } else {
      // 获取所有策略的统计
  ...
  compareStrategies(strategyAId: string, strategyBId: string): any {
    ...
    return strategySignalManager.compareSignals(resultA.signals, resultB.signals)
  }
   ...
   * 获取指定时间范围内的信号
   */
  getSignalsByTimeRange(strategyId: string, startTime: number, endTime: number): StrategySignal[] {
    const result = this.lastSignalResults.get(strategyId)
    if (!result) {
      return []
    }
  ...
  async batchUpdateStrategies(
    ...
    for (const { strategyId, params } of strategyUpdates) {
      try {
        const diff = await this.updateStrategyParams(strategyId, params, symbol, period)
        if (diff) {
