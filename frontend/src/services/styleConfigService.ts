...
export class StyleConfigService {
   ...
   * 获取策略信号样式
   */
  getSignalStyle(strategyId: string, signalType: SignalType, confidence: number = 100): SignalMarkerStyle {
    // 1. 检查用户自定义样式
    ...
    if (presetStrategy && presetStrategy.styles[signalType]) {
      return this.applyConfidenceAdjustment(presetStrategy.styles[signalType], confidence)
    }

    // 3. 使用当前主题的默认样式
    const currentTheme = this.themes.get(this.currentTheme)
    if (currentTheme && currentTheme.signalStyles[signalType]) {
      return this.applyConfidenceAdjustment(currentTheme.signalStyles[signalType], confidence)
    }

    // 4. 最后的回退样式
    return this.applyConfidenceAdjustment(DEFAULT_SIGNAL_THEMES.light[signalType], confidence)
  }
   ...
   * 更新用户样式配置
   */
  updateUserStyleConfig(strategyId: string, signalType: SignalType, style: Partial<SignalMarkerStyle>): void {
   ...
   * 启用/禁用用户样式配置
   */
  toggleUserStyleConfig(strategyId: string, signalType: SignalType, enabled: boolean): void {
    const key = this.generateConfigKey(strategyId, signalType)
   ...
   * 获取用户样式配置
   */
  getUserStyleConfig(strategyId: string, signalType: SignalType): UserStyleConfig | null {
    const key = this.generateConfigKey(strategyId, signalType)
  ...
  applyStylePreset(presetId: string): void {
    ...
    // 应用预设配置
    for (const [strategyId, signalStyles] of Object.entries(preset.strategies)) {
  ...
  createStylePreset(
    ...
    const strategies: Record<string, Record<SignalType, SignalMarkerStyle>> = {}

    // 从当前用户配置创建预设
    for (const config of this.userConfigs.values()) {
      if (!strategies[config.strategyId]) {
        strategies[config.strategyId] = {} as Record<SignalType, SignalMarkerStyle>
      }
      strategies[config.strategyId][config.signalType] = config.style as SignalMarkerStyle
    }
  ...
  importStyles(exportedStyles: {
    ...
    if (exportedStyles.userConfigs) {
      for (const config of exportedStyles.userConfigs) {
        const key = this.generateConfigKey(config.strategyId, config.signalType)
        this.userConfigs.set(key, config)
      }
    }
   ...
   * 私有方法：应用置信度调整
   */
  private applyConfidenceAdjustment(baseStyle: SignalMarkerStyle, confidence: number): SignalMarkerStyle {
    const adjustedStyle = { ...baseStyle }

    // 根据置信度调整透明度
    adjustedStyle.opacity = Math.max(0.3, Math.min(1, (confidence / 100) * 0.7 + 0.3))
   ...
   * 私有方法：生成配置键
   */
  private generateConfigKey(strategyId: string, signalType: SignalType): string {
