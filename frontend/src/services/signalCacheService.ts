...
export class SignalCacheService {
  ...
  async incrementalUpdate(
    ...
    // 检测参数变化
    const changes = this.detectParameterChanges(
      strategyId,
      oldParams,
      newParams,
    );
  ...
  async warmupCache(
    ...
    const promises = strategies.map(
      async ({ strategyId, params, symbol, period }) => {
        const key = this.generateCacheKey(
          'strategy_signals',
          strategyId,
          params,
  ...
  getCacheStats(): {
    ...
    const totalRequests = Array.from(this.accessStats.values()).reduce(
  ...
  private setCacheItem(
    key: string,
    item: Omit<ExtendedCacheItem, 'key'>,
  ): void {
    // 检查缓存大小限制
    if (this.cache.size >= this.config.maxEntries) {
      this.evictLeastRecentlyUsed();
  ...
  private isCriticalParameter(strategyId: string, paramName: string): boolean {
    ...
    const strategyType = presetStrategies.includes(strategyId)
      ? (strategyId as StrategyType)
      : 'custom';
  ...
  private async performIncrementalUpdate(
    ...
    switch (strategyId) {
      case 'sma_crossover':
        return this.performSMAIncrementalUpdate(
          oldParams,
          newParams,
          oldResult,
          computeNewSignals,
