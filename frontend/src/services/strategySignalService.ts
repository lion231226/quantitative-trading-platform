...
import { CandlestickData, KlineData } from '../types/kline.types';
...
export class StrategySignalManager implements IStrategySignalManager {
  ...
  constructor(
    ...
    defaultTTL: number = DEFAULT_CACHE_TTL,
  ) {
  ...
  async loadSignals(
    ...
    try {
      // 检查缓存
      const cacheKey = StrategySignalUtils.generateCacheKey(
        strategyId,
        params,
        symbol,
        period,
          ...
          dataRange: {
            start:
              data.candlesticks.length > 0
                ? new Date(data.candlesticks[0].timestamp).getTime()
                : 0,
            end:
              data.candlesticks.length > 0
                ? new Date(
                    data.candlesticks[data.candlesticks.length - 1].timestamp,
                  ).getTime()
                : 0,
          },
          params,
  ...
  async updateSignals(chartId: string, strategyId: string): Promise<void> {
    try {
      ...
      console.error(
        `更新图表 ${chartId} 上的策略信号 ${strategyId} 失败:`,
        error,
      );
      throw error;
    }
  ...
  async removeSignals(chartId: string, strategyId: string): Promise<void> {
    try {
      ...
      console.error(
        `移除图表 ${chartId} 上的策略信号 ${strategyId} 失败:`,
        error,
      );
      throw error;
    }
   ...
   */
  calculateDifference(
    oldSignals: StrategySignal[],
    newSignals: StrategySignal[],
  ): SignalDifference {
  ...
  getStatistics(signals: StrategySignal[]): SignalStatistics {
    ...
    signals.forEach((signal) => {
      ...
      // 策略统计
      strategyCounts[signal.strategyId] =
        (strategyCounts[signal.strategyId] || 0) + 1;
   ...
   * 私有方法：SMA金叉死叉策略
   */
  private calculateSMACrossoverSignals(
    data: KlineData,
    config: StrategyConfig,
    params: StrategyParams,
  ): StrategySignal[] {
   ...
   * 私有方法：RSI超卖策略
   */
  private calculateRSIOversoldSignals(
    data: KlineData,
    config: StrategyConfig,
    params: StrategyParams,
  ): StrategySignal[] {
   ...
   * 私有方法：计算金叉置信度
   */
  private calculateGoldenCrossConfidence(
    shortMA: number,
    longMA: number,
  ): number {
   ...
   * 私有方法：计算死叉置信度
   */
  private calculateDeathCrossConfidence(
    shortMA: number,
    longMA: number,
  ): number {
    const ratio = shortMA / longMA;
  ...
  private applyFilter(
    ...
    return signals.filter((signal) => {
      ...
      if (
        filter.signalTypes &&
        !filter.signalTypes.includes(signal.signalType)
      ) {
        return false;
      }
      ...
      // 策略类型过滤
      if (
        filter.strategyTypes &&
        !filter.strategyTypes.includes(signal.strategyType)
      ) {
        return false;
      }
   ...
   * 占位方法：其他策略类型实现
   */
  private calculateEMACrossoverSignals(
    data: KlineData,
    config: StrategyConfig,
    params: StrategyParams,
  ): StrategySignal[] {
  ...
  private calculateRSIOverboughtSignals(
    data: KlineData,
    config: StrategyConfig,
    params: StrategyParams,
  ): StrategySignal[] {
  ...
  private calculateMACDCrossoverSignals(
    data: KlineData,
    config: StrategyConfig,
    params: StrategyParams,
  ): StrategySignal[] {
  ...
  private calculateBollingerBandsSignals(
    data: KlineData,
    config: StrategyConfig,
    params: StrategyParams,
  ): StrategySignal[] {
