...
interface UseStrategySignalsReturn {
  ...
  activateStrategy: (strategyId: string, params: StrategyParams) => Promise<void>
  deactivateStrategy: (strategyId: string) => Promise<void>
  updateStrategyParams: (strategyId: string, params: StrategyParams) => Promise<SignalDifference | null>
...
// 自定义Hook：管理策略信号
export function useStrategySignals(config: UseStrategySignalsConfig): UseStrategySignalsReturn {
  const {
    symbol,
    period,
    chartId,
  ...
  // 状态管理
  const [activeStrategies, setActiveStrategies] = useState<Map<string, StrategyParams>>(new Map())
  const [signals, setSignals] = useState<Map<string, StrategySignalResult>>(new Map())
  ...
  // 激活策略
  const activateStrategy = useCallback(async (strategyId: string, params: StrategyParams) => {
    try {
      setLoading(true)
      setError(null)
      ...
      setError(`激活策略失败: ${err instanceof Error ? err.message : '未知错误'}`)
  ...
  const deactivateStrategy = useCallback(async (strategyId: string) => {
    try {
      ...
      setError(`停用策略失败: ${err instanceof Error ? err.message : '未知错误'}`)
  ...
  // 更新策略参数
  const updateStrategyParams = useCallback(async (strategyId: string, params: StrategyParams) => {
    try {
      setLoading(true)
      setError(null)
      ...
      // 更新策略参数
      const difference = await klineDataService.updateStrategyParams(strategyId, params, symbol, period)
      ...
      setError(`更新策略参数失败: ${err instanceof Error ? err.message : '未知错误'}`)
  ...
  const refreshSignals = useCallback(async () => {
    try {
      ...
      setError(`刷新信号失败: ${err instanceof Error ? err.message : '未知错误'}`)
  ...
  // 过滤信号
  const filterSignals = useCallback((filter: SignalFilter) => {
    const filteredSignals = new Map<string, StrategySignal[]>()

    for (const [strategyId, signalResult] of signals) {
      const filtered = strategySignalManager.optimizeSignals(signalResult.signals, filter)
  ...
  // 获取最近信号
  const getRecentSignals = useCallback((strategyId: string, count: number = 10) => {
  ...
  // 获取统计信息
  const getStatistics = useCallback((strategyId?: string) => {
    return klineDataService.getSignalStatistics(strategyId)
  }, [signals])

  // 比较策略
  const compareStrategies = useCallback((strategyAId: string, strategyBId: string) => {
    return klineDataService.compareStrategies(strategyAId, strategyBId)
  }, [signals])

  // 获取性能指标
  const getPerformance = useCallback((strategyId: string) => {
...
export function usePresetStrategies() {
  const [strategies, setStrategies] = useState<Record<string, StrategyConfig>>(PRESET_STRATEGIES)
